...
void interrupt  newint88(...) { print(); oldint70(); }	  // set function for service interrupt	int70
void interrupt  newint89(...) { print(); oldint71(); }	  // set function for service interrupt	int71
void interrupt  newint8A(...) { print(); oldint72(); }	  // set function for service interrupt	int72
void interrupt  newint8B(...) { print(); oldint73(); }	  // set function for service interrupt	int73
void interrupt  newint8C(...) { print(); oldint74(); }	  // set function for service interrupt	int74
void interrupt  newint8D(...) { print(); oldint75(); }	  // set function for service interrupt	int75
void interrupt  newint8E(...) { print(); oldint76(); }	  // set function for service interrupt	int76
void interrupt  newint8F(...) { print(); oldint77(); }	  // set function for service interrupt	int77
//---------------------------------------NEW INTERRUPTIONS--------------------------------------------
...
void initialize()
{
	...
	setvect(0x88, newint08);		// set new handler for IRQ 0 (adress newint08 to 0x88)
	setvect(0x89, newint09);		// set new handler for IRQ 1
	setvect(0x8A, newint0A);		// set new handler for IRQ 2 
	setvect(0x8B, newint0B);		// set new handler for IRQ 3 
	setvect(0x8C, newint0C);		// set new handler for IRQ 4 
	setvect(0x8D, newint0D);		// set new handler for IRQ 5 
	setvect(0x8E, newint0E);		// set new handler for IRQ 6 
	setvect(0x8F, newint0F);		// set new handler for IRQ 7 

	//set new handlers for IRQ 8-15
	setvect(0x08, newint88);		// set new handlers for IRQ 8
	setvect(0x09, newint89);		// set new handlers for IRQ 9
	setvect(0x0A, newint8A);		// set new handlers for IRQ 10
	setvect(0x0B, newint8B);		// set new handlers for IRQ 11
	setvect(0x0C, newint8C);		// set new handlers for IRQ 12
	setvect(0x0D, newint8D);		// set new handlers for IRQ 13
	setvect(0x0E, newint8E);		// set new handlers for IRQ 14
	setvect(0x0F, newint8F);		// set new handlers for IRQ 15
	//------------------------------------NEW HANDLERS--------------------------------------------------

	_disable(); // CLI

	// interrupt initializtion for Master
	outp(0x20, 0x11);				// ICW1 - initialize MASTER (0x11 to 0x20)
	outp(0x21, 0x88);				// ICW2 - base vector for MASTER (88H)
